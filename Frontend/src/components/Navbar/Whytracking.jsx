import React from 'react'
import styles from './Whytracking.module.css'

const WhyTracking = () => {
  return (
    <>

    <div  className={styles.Whytracking_nain_mainContainer_mydiv}>
    <div>
            <div className={styles.Whytracking_child_P_child_mainContainer_mydiv}>
                <p >
                TIME TRACKING FOR EVERY TEAM SIZE
                </p>  
            </div>
        <div className={styles.Whytracking_child_mainContainer_mydiv} >
            <div className={styles.Whytracking_child_child_mainContainer_mydiv}>
                <div>
                    <img src="data:image/svg+xml;base64," alt="" />
                </div>
                <div>
                    <h5>
                    For freelancers and solo users
                    </h5>
                    <p>
                    For solo entrepreneurs with a growing list of clients
                    </p>
                </div>
            </div>
            <div className={styles.Whytracking_child_child_mainContainer_mydiv}>
                <div>
                    <img src="data:image/svg+xml;base64," alt="" />
                </div>
                <div>
                    <h5>
                        For small teams
                    </h5>
                    <p>
                    For teams of less than 10 that need to track projects and bill clients
                    </p>
                </div>
            </div>
            <div className={styles.Whytracking_child_child_mainContainer_mydiv}>
                <div>
                    <img src="data:image/svg+xml;base64," alt="" />
                </div>
                <div>
                    <h5>
                    For big teams
                    </h5>
                    <p>
                    For growing teams that need robust and flexible solutions for time and project tracking
                    </p>
                </div>
            </div>
           
        </div>
         <div  className={styles.Whytracking_child_child_surveillance_mydiv}>
            <button>
              Toggl Track stands against employee surveillance. Read our statement here.
              <svg viewBox="0 0 34 16"><g fill="none"><path fill="#412A4C" d="M2 7v2h32V7z"></path><path d="M7.5 2L1.677 7.823M7.5 14L1.677 8.177" stroke="#412A4C" stroke-width="2" stroke-linecap="square"></path></g></svg>
            </button>
         </div>
        </div>

        
    </div>
    </>
  )
}

export default WhyTracking